...
class User(Base):
    ...
    join_time: Mapped[datetime] = mapped_column(TIMESTAMP(timezone=True), init=False, default_factory=timezone.now, comment='注册时间')
    last_login_time: Mapped[datetime | None] = mapped_column(TIMESTAMP(timezone=True), init=False, onupdate=timezone.now, comment='上次登录')
    # 部门用户一对多
